...
#define LABEL_NONE 0		   /* Means sysd hasnt set a label */
...
typedef struct {
	...
	Handle
	    matched_index; /* index into the caller's handle array; WAITANY_NO_MATCH on timeout */
	...
	Marker marker; /* Added in 1.1: waitany on 32-bit architectures is the only syscall that can
			  use markers */
	Label label;   /* Added in 1.1.1: sysd assigned label helps nameserver create an unforgeable
			  identity. */
} WaitanyResult;
