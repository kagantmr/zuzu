...
/* (size) -> id or -err */
static inline int32_t _memshare(size_t size) {
...
/* (id, addr) -> addr or -err */
static inline void* _attach(int32_t id, void *addr) {
    register int32_t   r0 __asm__("r0") = id;
    register uintptr_t r1 __asm__("r1") = (uintptr_t)addr;
    __asm__ volatile("svc %[num]"
                     : "+r"(r0)
                     : "r"(r1), [num] "i"(SYS_ATTACH)
                     : "memory");
