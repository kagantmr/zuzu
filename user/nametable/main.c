...
static int nt_lookup(uint32_t name_u32, uint32_t *out_handle) {
    for (int i = 0; i < NT_MAX_SERVICES; i++) {
        if (registry_table[i].handle != 0 && name_equals_u32(registry_table[i].name, name_u32)) {
...
int main(void) {
    ...
    while (1) {
        ...
        if (command == NT_REGISTER) {
            status = nt_register(name_u32, arg);
        } else if (command == NT_LOOKUP) {
            status = nt_lookup(name_u32, &out_handle);
            if (status == NT_LU_OK) {
        ...
        _reply(sender, (uint32_t)status, out_handle, 0);
    }
}