...
int tcp_output(TcpPcb *pcb, uint8_t flags, const uint8_t *data,
  ...
  size_t occupied =
      (pcb->nranges ? pcb->ranges[pcb->nranges - 1].end : pcb->rcv_nxt) -
      pcb->rcv_rsq;
  ...
  th->checksum =
      htons(tcp_checksum(pcb->local_ip, pcb->remote_ip, buf, seg_len));
  ...
  pcb->snd_nxt +=
      data_len + ((flags & TCP_SYN) ? 1 : 0) + ((flags & TCP_FIN) ? 1 : 0);
...
void tcp_rto_cb(void *arg) {
  ...
  pcb->rtt_timing =
      false; /* Karn: timed segment now ambiguous, drop the sample */
  pcb->rto_ms *= 2;
...
int tcp_send(int idx, const uint8_t *data, uint16_t len) {
  ...
  size_t off = (pcb->snd_una + pcb->buffered_bytes) &
               (TCP_SND_BUF - 1); // where the write starts
  size_t first =
      MIN(n, TCP_SND_BUF - off); // bytes before hitting the physical end
...
void tcp_send_rst(ipv4_addr_t src_ip, ipv4_addr_t dst_ip,
  ...
  if (seg->flags & TCP_ACK) {
    ...
    uint32_t seg_len = seg->payload_len + ((seg->flags & TCP_SYN) ? 1 : 0) +
