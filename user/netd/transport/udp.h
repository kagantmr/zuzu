...
_Static_assert(sizeof(udp_hdr_t) == 8, "udp_hdr_t size");

#define UDP_MAX_TABLE 64
#define UDP_MAX_PAYLOAD (NIC_MTU - 20 - 8)   // 1472

typedef void (*udp_handler_t)(ipv4_addr_t src_ip, port_t src_port,
                              port_t dst_port, const uint8_t *data, uint16_t len);
void udp_init(void);
int  udp_bind(port_t port, udp_handler_t handler);
