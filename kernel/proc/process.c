...
ProcessObj *KernelProcessLoad(const void *zxf_data, size_t zxf_size, const char *name,
...
void ProcessKill(ProcessObj *p, const int exit_status)
{
    ...
    if (p->flags & (PROC_FLAG_INIT | PROC_FLAG_DEVMGR))
    {
        if (((uint32_t)exit_status & FATAL_TAG_MASK) == FATAL_TAG)
        {
            ...
            panic("critical process '%s' (pid %u) exited: %s", p->name, (unsigned)p->pid,
                  fatal_reason_str((uint32_t)exit_status & FATAL_REASON_MASK));
        }
        else
        {
            /* unexpected death, or exit with no reason */
            panic("critical process '%s' (pid %u) died unexpectedly (status %d)", p->name,
                  (unsigned)p->pid, exit_status);
        }
    }
