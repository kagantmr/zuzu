...
# TARGET). Defined here, not next to its build rule near the bottom of this
# file, because it's referenced as a *prerequisite* by the U-Boot uImage
# rule above that rule's own definition — prerequisite lists expand at
# parse time (unlike recipe bodies, which expand lazily at run time), so a
# forward reference here would silently expand to empty and drop from the
# dependency list.
IMG            = build/zuzu.img
...
uboot:
	...
	@if [ "$(UNAME_S)" = "Darwin" ]; then \
	    $(MAKE) -C $(UBOOT_SRC_DIR) CROSS_COMPILE=$(CROSS) HOSTLDFLAGS="-Wl,-ld_classic" \
	        -j$$(sysctl -n hw.ncpu) > build/uboot-build.log 2>&1 || true; \
	else \
...
# IMG itself is defined up near TARGET/MAP — see the comment there.
$(IMG): $(TARGET)
	@echo "  OBJCOPY $@"
