...
uint32_t dtb_get_reg_addr(const dtb_node_t* root, const char* path)
{
    ...
    for (uint32_t i = 0; i < node->property_count; i++) {
        if (strcmp(node->properties[i].name, "reg") == 0) {
            ...
            const uint8_t* val = node->properties[i].value;

            if (node->properties[i].length < 4)
                return 0;

            uint32_t be = *(uint32_t*)val;   // still big-endian
            return bswap32(be);
        }
    }
...
uint32_t dtb_get_reg_size(const dtb_node_t* root, const char* path)
{
    ...
    for (uint32_t i = 0; i < node->property_count; i++) {
        if (strcmp(node->properties[i].name, "reg") == 0) {
            ...
            if (node->properties[i].length < 8)
                return 0;

            uint32_t be = *(uint32_t*)(val + 4); // second cell
            return bswap32(be);
        }
    }
